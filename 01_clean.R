...
vf_tele_data <- vf_tele_data %>% 
  mutate(clean_date = as.POSIXct(dt, format="%Y-%m-%dT%H:%M:%SZ", tz="UTC") + 12 * 3600,
...
sa2_codes_names <- rename(sa2_codes_names, area_code = rowname,
...
urban_rural_to_sa2 <- rename(urban_rural_to_sa2,
                             area_code = Concordance.report, location_3 = X.3)
